...
public class hw2 {
	...
	private static double sumHelper(double[] list, int i) {
		if ( list == null || list.length == 0 || i >= list.length )
			return 0.0;
		return sumHelper(list, i+1) + list[i];
	}
	...
	private static void reverseHelper(double[] list, int lo, int hi) {
		if(list==null || list.length==0 || list.length==1 || lo>=hi)
			return ;
		double tmp = list[lo];
		list[lo] = list[hi];
		list[hi] = tmp;
		reverseHelper(list, lo+1, hi-1);
	}
	 ...
	 * you saw:
	 *
	 * OUTPUT: terribleFibonacci(57)=365435296162 // TODO
	 *
	...
	private static long fibonacciHelper(int n, long prev, long curr) {
		if(n==0)return prev;
		if(n==1)return curr;
