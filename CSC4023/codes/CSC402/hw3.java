...
public class hw3 
{
	public static class MyLinked 
	{
		...
		public int sizeForward () {
			// return StdRandom.uniform (100); //TODO: fix this
			Node head = this.first;
			return sizeForwardHelper(head, 0);
		}
		private int sizeForwardHelper(Node head,int size) {
			if (head==null) return size;
			return sizeForwardHelper(head.next, size+1);
		}
		...
		public int sizeBackward () {
			// return StdRandom.uniform (100); //TODO: fix this
			Node head = this.first;
			return sizeBackwardHelper(head);
		}
		private int sizeBackwardHelper(Node head) {
			if (head==null) return 0;
			return sizeBackwardHelper(head.next) + 1;
		}
		...
		public int positionOfLastFiveFromEnd () {
			// return StdRandom.uniform (100); //TODO: fix this
			Node head = this.first;
			return lastFiveHelper(head);
		}
		private int lastFiveHelper(Node head) {
			if (head==null) return -1;
			int res = lastFiveHelper(head.next);
			if(res<0 && head.item==5.0)res=-res-1;
			else if(res<0)res--;
			return res;
		}
		...
		private Node reverseHelper(Node head) {
			...
			Node new_head = reverseHelper(head.next);
			head.next.next = head;
			head.next = null;
			return new_head;
		}
