...
//		return 0;
		Node ptr = this.root;
		return sizeOddHelper(ptr);
	}
	private int sizeOddHelper(Node node) {
		if(node==null) return 0;
		return sizeOddHelper(node.left)+sizeOddHelper(node.right)+(node.key%2);
	}
...
//		return 0;
		Node ptr = this.root;
		return sizeAtDepthHelper(ptr, 0, k);
	}
	private int sizeAtDepthHelper(Node node, int d, int depth) {
		if(node==null || d>depth) return 0;
		return sizeAtDepthHelper(node.left,d+1,depth) + sizeAtDepthHelper(node.right,d+1,depth) + (d==depth?1:0);
	}
...
//		return 0;
		Node ptr = this.root;
		return sizeAboveDepthHelper(ptr, 0, k);
	}
	private int sizeAboveDepthHelper(Node node,int d,int depth) {
		if(node==null || d>=depth) return 0;
		return sizeAboveDepthHelper(node.left,d+1,depth) + sizeAboveDepthHelper(node.right,d+1,depth) + 1;
	}
...
//		return 0;
		Node ptr = this.root;
		return sizeBelowDepthHelper(ptr, 0, k);
	}
	private int sizeBelowDepthHelper(Node node,int d, int depth) {
		if(node==null) return 0;
		return sizeBelowDepthHelper(node.left,d+1,depth) + sizeBelowDepthHelper(node.right,d+1,depth) + (d>depth?1:0);
	}
...
//		return false;
		Node ptr = this.root;
		return isPerfectlyBalancedSHelper(ptr)!=-1;
	}
	private int isPerfectlyBalancedSHelper(Node node) {
		if(node==null) return 0;
		int sizeleft = isPerfectlyBalancedSHelper(node.left);
		int sizeright = isPerfectlyBalancedSHelper(node.right);
		if(sizeleft!=-1 && sizeleft==sizeright) return sizeleft*2+1;
		else return -1;
	}
...
//		return false;
		Node ptr = this.root;
		return isPerfectlyBalancedHHelper(ptr)!=-2;
	}
	private int isPerfectlyBalancedHHelper(Node node) {
		if(node==null) return 0;
		int highleft = isPerfectlyBalancedHHelper(node.left);
		int highright = isPerfectlyBalancedHHelper(node.right);
		if(highleft!=-2 && highleft==highright) return highleft+1;
		else return -2;
	}
...
//		return false;
		Node ptr = this.root;
		return isOddBalancedHelper(ptr)!=-1;
	}
	private int isOddBalancedHelper(Node node) {
		if(node==null) return 0;
		int oddleft = isOddBalancedHelper(node.left);
		int oddright = isOddBalancedHelper(node.right);
		if(oddleft!=-1 && oddleft==oddright) {
			if(node.key%2==1) return oddleft*2+1;
			else return oddright*2;
		}else return -1;
	}
...
//		return false;
		Node ptr = this.root;
		return isSemiBalancedHelper(ptr)!=-1;
	}
	private int isSemiBalancedHelper(Node node) {
		if(node==null) return 0;
		int sizeleft = isSemiBalancedHelper(node.left);
		int sizeright = isSemiBalancedHelper(node.right);
		boolean flag;
		if(sizeleft>=sizeright) {
			if(sizeleft<=sizeright*3)flag=true;
			else flag=false;
		}else {
			if(sizeright<=sizeleft*3)flag=true;
			else flag=false;
		}
		if(sizeleft!=-1 && flag) return sizeleft*2+1;
		else return -1;
	}
	...
	public void removeOddSubtrees () {
		// TODO
		this.root = removeOddSubtreesHelper(this.root);
	}
	private Node removeOddSubtreesHelper(Node node) {
		if(node==null || node.key%2==1) return null;
		node.left = removeOddSubtreesHelper(node.left);
		node.right = removeOddSubtreesHelper(node.right);
		return node;
	}
	...
	public void removeBelowDepth(int k) {
		// TODO
		this.root = removeBelowDepthHelper(this.root,k);
	}
	private Node removeBelowDepthHelper(Node node, int d) {
		if(node==null||d<0) return null;
		node.left = removeBelowDepthHelper(node.left,d-1);
		node.right = removeBelowDepthHelper(node.right,d-1);
		return node;
	}
	...
	public void addZeroToSingles() {
		// TODO
		Node ptr = this.root;
		addZeroToSinglesHelper(ptr);
	}
	private void addZeroToSinglesHelper(Node node) {
		if(node==null) return ;
	...
	public void removeLeaves() {
		// TODO
		this.root = removeLeavesHelper(this.root);
	}
	private Node removeLeavesHelper(Node node) {
	...
	public void removeSingles() {
		// TODO
		this.root = removeSinglesHelper(this.root);
	}
	private Node removeSinglesHelper(Node node) {
		if(node==null) return null;
		if(node.left==null && node.right==null) return node;
		else if(node.left==null) return removeSinglesHelper(node.right);
		else if(node.right==null) return removeSinglesHelper(node.left);
		node.left = removeSinglesHelper(node.left);
		node.right = removeSinglesHelper(node.right);
		return node;
	}
