import scala.annotation.tailrec // GENERATED
/* INSTRUCTIONS
 *
...
//    throw UnsupportedOperationException()
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    if xs.isEmpty then return Nil
    removeDupes2Aux(xs.tail,xs.head,1)
    // TODO: Provide definition here.
//    throw UnsupportedOperationException()
  def removeDupes2Aux[X](xs: List[X],e: X,cnt: Int): List[(Int, X)] =
    xs match
      case Nil => (cnt, e)::Nil
      case y::ys if y!=e => (cnt,e)::removeDupes2Aux(ys,y,1)
      case y::ys => removeDupes2Aux(ys,y,cnt+1)
